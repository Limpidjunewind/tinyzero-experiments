...
"""
PPO trainer v3: Outcome Reward (v2) + Process Reward Model (PRM).
Uses countdown_v2 for outcome scoring and a trained PRM for step-level scoring.
"""
...
def main_task(config):
    ...
    # Initialize PRM scorer (runs on CPU to save GPU memory)
    # Import here inside Ray worker where sys.path is set
    sys.path.insert(0, '/data/fangda/tinyzero/prm')
    from integrate import PRMScorer

    prm_scorer = PRMScorer(
        checkpoint_path=PRM_CHECKPOINT,
        device='cpu',
        prm_weight=PRM_WEIGHT
    )
    print(f"[PRM] Loaded from {PRM_CHECKPOINT}, weight={PRM_WEIGHT}")
