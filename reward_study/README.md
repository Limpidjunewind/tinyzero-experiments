...
1. 安装 veRL 框架：https://github.com/volcengine/verl
2. 下载 Qwen2.5-3B 到本地路径（下面的 `BASE_MODEL`）
3. 下载 Countdown-Tasks-3to4 数据集到 `DATA_DIR`。格式：parquet，列名 `prompt`、`reward_model`（内含 `ground_truth.target` 和 `ground_truth.numbers`）
4. 4 张 GPU（≥24GB/卡），实际训练用的是 L40S 46GB
...
**重要教训**（详见 `analysis/v5b_results.md`）：v5b 看起来 val_score 有 0.924，但真实正确率是 0%——模型学会了 `(given_a - given_b) / given_c + TARGET` 这个 reward hacking 模板。**训练前务必跑 `test_countdown_vXX.py` 式的对抗测试**。
...
**PRM 迭代故事**（完整分析见 `docs/experiment_report.md` §2.3.1）：
1. **v1**：简单数据 + 字符级 tokenizer → 69.16%
2. **v2**：加 NumberValueEncoder + 困难数据 → **53.31%（失败！）** —— 归纳偏置（让 embedding 上 44 ≈ 45）结构性地破坏了 off-by-1 判别能力
3. **v1_improved**：回退架构到 v1，保留困难数据 + 延长训练 → **73.68%（最终采用）**

v2 的失败是干净的"架构归纳偏置 vs 任务判别粒度"案例研究。
