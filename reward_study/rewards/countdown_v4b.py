"""
Reward function v4b: Strongly strengthened number usage constraint.

Based on v2, with number usage score weight increased from 0.05 to 0.3.
This is the aggressive variant (vs v4a's 0.15). At 0.3, the number usage
weight is comparable to the proximity score (0.4), creating a strong
incentive to use the correct numbers.

Risk: May cause optimization conflict between number usage (0.3) and
proximity (0.4) -- the model might sacrifice arithmetic accuracy to
satisfy the number constraint.

Changes vs v2:
- Number usage score: 0.05 -> 0.3 (full match)
- Partial number usage: 0.025 -> 0.15 (max partial credit)
...
def compute_score(solution_str, ground_truth, method='strict', format_score=0.1, score=1.):
    ...
    - Number usage score (correct numbers used):       0.3
    ...
    # --- 3. Number usage score (v4b: weight 0.3, up from v2's 0.05) ---
    ...
    if numbers_valid:
        ...
        if total > 0:
            total_score += 0.3 * (matched / total) * 0.5  # max 0.15 for partial
